...
def _get_ymd_from_date_string(date_string):
    if isinstance(date_string, datetime):
        date_string = to_query_date(date_string)
...
def to_query_daily_data(code, start_date_string, end_date_string, ktype="D", is_print=False):
    if is_print:
        print("to_query_daily_data(", code, ",", start_date_string, ",", end_date_string, ")")
    if isinstance(code, int):
        code = "%06d" % code

    start_date_string = to_query_date(start_date_string)
    end_date_string = to_query_date(end_date_string)
    ...
    def is_need_query_again(length):
        is_needquery_daily_data = pydash.is_equal(ktype, "D") and length == 1
        is_need_query_5min_data = pydash.is_equal(ktype, "5") and length == (60 / 5)
        return is_needquery_daily_data or is_need_query_5min_data
    ...
    if new_start_date < query_end_date:
        new_result = to_query_daily_data(code, new_start_date, query_end_date, ktype)
...
def get_daily_data(code, start_string="2018-01-01", just_download=False):
    ...
    q_data = to_query_daily_data(code, start_string, _end_string, ktype)
